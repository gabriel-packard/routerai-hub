    ...
    <div className="mx-auto max-w-6xl space-y-8">
      <section className="rounded-3xl beach-gradient p-8 md:p-12 text-primary-foreground shadow-xl relative overflow-hidden">
        ...
        <div className="relative">
          <Badge className="bg-white/20 text-primary-foreground border-0 mb-3">🌴 Beach mode</Badge>
          <h1 className="text-3xl md:text-5xl font-bold tracking-tight">
            Your beachfront hub for every AI tool
          </h1>
          <p className="mt-3 max-w-2xl text-primary-foreground/90 text-lg">
            Discover top AI platforms, chat, draft emails, summarize meetings, plan your day,
            and review Python — all under one sunny roof.
          </p>
        ...
        >
          {filtered.map((cat) => (
            <AccordionItem
              ...
              value={cat.key}
              id={`cat-${cat.key}`}
              className="border rounded-2xl bg-card px-4 shadow-sm data-[state=open]:shadow-md"
            >
              ...
              <AccordionContent>
                <div className="grid gap-4 md:grid-cols-2 pt-2">
                  {cat.tools.map((tool) => (
                    <Card key={tool.name} className="ai-card border-2 border-transparent hover:border-turquoise/60">
                      <CardHeader>
...
// Exposed for the Python tab redirect
